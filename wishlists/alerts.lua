...
function GoWWishlists:CreateAlertItemRow(parent, match, itemLink)
    local row = self:CreateWishlistCardRow(parent, {
        rowHeight = self:GetAlertItemRowHeight(),
        compactIconSize = 24,
        compactInnerHeight = 38,
        leadingAccent = { width = 3 },
    });
    row.entry = match;
    row.itemLink = itemLink;
    self:PopulateItemRow(row, match, itemLink);
    return row;
end
...
function GoWWishlists:RelayoutAlertContainer(frame)
    ...
    for _, row in ipairs(frame.itemRows) do
        ...
        row:Show();
        yOffset = yOffset + self:GetAlertItemRowHeight();
    end
...
function GoWWishlists:OnStartLootRoll(rollID)
    ...
    C_Timer.After(0.1, function()
        ...
        local matches = GoWWishlists:FindAllWishlistMatches(itemId, itemLink);
        if matches then
